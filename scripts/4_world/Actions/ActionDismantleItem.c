class ActionDismantleItemCB : ActionContinuousBaseCB
{
	...
	{
		m_ActionData.m_ActionComponent = new CAContinuousTime( DMP_UAConstants.DMP_TimeSpent_DEFAULT_DECONSTRUCT / 2 );
	}
};
	...
	{	
		...
		{				
			...
			if(myItem)
			{
				...
				if (myItem.IsInherited(DMP_OpenableFurniture) || myItem.IsInherited(DMP_UnOpenableFurniture) || myItem.IsInherited(dmp_stove_Base))								
					return true;
			}
		}
		return false;
